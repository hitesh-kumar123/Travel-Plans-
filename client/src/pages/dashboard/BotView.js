...
const BotView = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/bot/recommend",
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", text: response.data.reply, data: response.data },
      ]);
    } catch (error) {
      setMessages((prev) => [
        ...prev,
        {
          role: "bot",
          text: "I am having trouble connecting to the database right now. Please try again.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  const handleSaveTripToDashboard = async (aiData) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post("http://localhost:5000/api/bot/save-trip", aiData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success(
        "✨ Trip and Expenses automatically added to your dashboard!",
      );
    } catch (error) {
    ...
    >
      ...
      <Box>
        <Typography
          variant="h5"
          fontWeight="700"
          sx={{ color: "#6B5B95", mb: 0.5 }}
        >
          AI Travel Concierge
        </Typography>
        <Typography variant="body2" color="text.secondary">
          Plan, optimize, and budget your next adventure through natural
          conversation.
        </Typography>
      </Box>
                    ...
                    >
                      {msg.data.destination && (
                        ...
                        >
                          ...
                          >
                            <Avatar
                              sx={{ bgcolor: "#e8eaf6", color: "#3B5998" }}
                            >
                              <PlaceRoundedIcon />
                            </Avatar>
                            <Box>
                              ...
                              <Typography variant="subtitle1" fontWeight="bold">
                                {msg.data.destination.name},{" "}
                                {msg.data.destination.city}
                              </Typography>
                            </Box>
                          </CardContent>
                        </Card>
                          ...
                          >
                            <CardContent sx={{ pb: "16px !important" }}>
                              ...
                              <Typography variant="body2">
                                {msg.data.flight.carrier} (
                                {msg.data.flight.type})
                              </Typography>
                              <Typography
                                variant="h6"
                                fontWeight="bold"
                                sx={{ mt: 1, color: "#2E384D" }}
                              >
                                ₹{msg.data.flight.price}
                              </Typography>
                            </CardContent>
                          </Card>
                        )}
                          ...
                          <CardContent>
                            ...
                            >
                              <TableBody>
                                {msg.data.expenses.map((exp, i) => (
                                  <TableRow key={i}>
                                    <TableCell
                                      sx={{ pl: 0, py: 1.5, color: "#4A5568" }}
                                    >
                                      {exp.item}
                                    </TableCell>
                                    <TableCell
                                      align="right"
                                      sx={{ pr: 0, py: 1.5, fontWeight: "500" }}
                                    >
                                      ₹{exp.cost}
                                    </TableCell>
                                  </TableRow>
                                ...
                                <TableRow>
                                  <TableCell
                                    sx={{ pl: 0, borderBottom: "none", pt: 2 }}
                                  >
                                  ...
                                  <TableCell
                                    align="right"
                                    sx={{ pr: 0, borderBottom: "none", pt: 2 }}
                                  >
                                    <Typography
                                      variant="h6"
                                      fontWeight="bold"
                            ...
                            >
                              <Button
                                ...
                                disableElevation
                                onClick={() =>
                                  handleSaveTripToDashboard(msg.data)
                                }
                                sx={{
                                  bgcolor: "#3B5998",
                                  borderRadius: "50px",

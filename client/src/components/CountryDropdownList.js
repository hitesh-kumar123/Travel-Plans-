...
const CountryDropdownList = ({ where, setWhere, destinations, setShowRecentSearches }) => {
  ...
  useEffect(() => {
    ...
    const uniqueNames = Array.from(new Set(matches.map(d => d.name).filter(Boolean)));
  ...
  useEffect(() => {
    const updatePosition = () => {
      if (isOpen) {
        const inputEl = document.querySelector('.wander-search-bar input.wander-sf-val');
        if (inputEl) {
      ...
      {filteredCountries.map((country, idx) => (
        <div
          ...
          style={{
            padding: '0.9rem 1rem',
            cursor: 'pointer',
            background: focusedIndex === idx ? 'rgba(26, 74, 107, 0.05)' : 'transparent',
            color: '#1a4a6b',
            fontWeight: 500,

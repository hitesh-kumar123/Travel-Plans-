...
console.log('- MongoDB: update the database user password in your provider (Atlas), then update `MONGO_URI` in .env with the new password.');
console.log('- Gmail: generate a new App Password at https://myaccount.google.com/apppasswords and set `EMAIL_PASS` in .env.');
console.log('- Resend/other SMTP: rotate API keys in provider console and update `EMAIL_PASS` accordingly.');
console.log('- After rotating provider credentials, restart the server and run the SMTP test:');
console.log('\n  node server/utils/test-smtp.js\n');

console.log('If you want, commit the backup to a secure location or move it to an off-repo safe store.');
console.log('Done.');

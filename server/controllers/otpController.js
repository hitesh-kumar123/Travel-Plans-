  ...
  try {
    ...
    jwt.sign(
      ...
      (err, token) => {
        if (err) throw err;
        res.json({
          msg: "Email verified successfully! ðŸŽ‰",
          token,
          user: { id: user.id, name: user.name, email: user.email },
        });
      },
...
exports.resendOtp = async (req, res, next) => {
  try {
    ...
    try {
      ...
      logEmailFailure("OTP resend", emailErr, "otpController");
    ...
    if (process.env.NODE_ENV === "development" || !process.env.SMTP_HOST) {
      console.log("\n=======================================================");
      console.log("ðŸš€ DEV MODE: EMAIL VERIFICATION OTP GENERATED");
...
exports.requestEmailChange = async (req, res, next) => {
  try {
    ...
    try {
      ...
      logEmailFailure("email change OTP", emailErr, "otpController");
      return res.status(500).json({
    ...
    if (process.env.NODE_ENV === "development" || !process.env.SMTP_HOST) {
      console.log("\n=======================================================");
      console.log("ðŸš€ DEV MODE: EMAIL CHANGE OTP GENERATED");
